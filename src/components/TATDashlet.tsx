      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 p-5">
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-md bg-sky-50 p-3 dark:bg-sky-950/40">
            <p className="text-micro font-semibold uppercase tracking-wide text-sky-700 dark:text-sky-300">Merchants</p>
            <p className="mt-1 text-2xl font-semibold tracking-tight tabular-nums text-sky-900 dark:text-sky-200">{overall.count}</p>
            <p className="mt-0.5 text-micro text-sky-700 dark:text-sky-300">live</p>
          </div>
          <div className="min-w-0 rounded-md bg-muted p-3">
            <p className="truncate text-micro font-semibold uppercase tracking-wide text-muted-foreground">Total {arrLabel}</p>
            <p className="mt-1 truncate text-2xl font-semibold tracking-tight tabular-nums text-foreground">{formatCr(overall.totalArr)}</p>
            <p className="mt-0.5 text-micro text-muted-foreground">Cr</p>
          </div>
          <div className="rounded-md bg-emerald-50 p-3 dark:bg-emerald-950/40">
            <p className="text-micro font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">Avg TAT</p>
            ...
            <p className="mt-0.5 text-micro text-emerald-700 dark:text-emerald-300">days</p>
          </div>
        </div>
        {rows.length === 0 ? (
                ...
                {rows.map(r => (
                  <div key={r.id} className="grid grid-cols-[1fr_auto] items-center gap-3 px-3 py-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-micro text-muted-foreground">{arrLabel}: {formatCr(r.arr)} Cr</p>
                    </div>
