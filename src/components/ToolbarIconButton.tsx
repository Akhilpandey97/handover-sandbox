...
>(({ icon, label, count, className, ...props }, ref) => (
  ...
  >
    {icon}
    {!!count && (
      <span className="absolute -right-1 -top-1 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-primary px-1 text-2xs font-semibold leading-none text-primary-foreground">
        {count}
      </span>
    )}
  </Button>
